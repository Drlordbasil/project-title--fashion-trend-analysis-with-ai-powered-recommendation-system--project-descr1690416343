...
def update_inventory(real_time_data):
    ...
    if "out_of_stock" in real_time_data:
        notify_luna("Out of stock for product: " +
                    real_time_data["out_of_stock"])
...
def main():
    ...
    trend_predictions = fashion_trend_analysis(
        "reviews.csv", "social_media_data.csv", "magazines_data.csv")
    print("Fashion Trend Predictions:", trend_predictions)

    # Run Personalized Recommendation System
    personalized_recommendations = personalized_recommendation_system(
        "customer_data.csv", "product_data.csv")
